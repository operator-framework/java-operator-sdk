...
public class Utils {
  ...
  public static WebPage createWebPageForStatusUpdate(WebPage webPage, String configMapName) {
    ...
    res.setMetadata(new ObjectMetaBuilder().withName(webPage.getMetadata().getName())
        .withNamespace(webPage.getMetadata().getNamespace()).build());
  ...
  public static Ingress makeDesiredIngress(WebPage webPage) {
    ...
    ingress.getSpec().getRules().get(0).getHttp().getPaths().get(0).getBackend().getService()
