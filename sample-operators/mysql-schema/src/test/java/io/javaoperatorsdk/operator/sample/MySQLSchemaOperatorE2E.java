...
class MySQLSchemaOperatorE2E {
  ...
  static {
    infrastructure
        .add(new NamespaceBuilder().withNewMetadata().withName(MY_SQL_NS).endMetadata().build());
    try {
  ...
  AbstractOperatorExtension operator = isLocal()
      ? LocallyRunOperatorExtension.builder().withReconciler(new MySQLSchemaReconciler()) // configuration
                                                                                          // for
                                                                                          // schema
                                                                                          // comes
                                                                                          // from
      ...
      : ClusterDeployedOperatorExtension.builder()
          ...
          .withInfrastructure(infrastructure).build();
  ...
  void test() {
    ...
    testSchema.setMetadata(new ObjectMetaBuilder().withName(TEST_RESOURCE_NAME)
    ...
    await().atMost(2, MINUTES).ignoreExceptions().untilAsserted(() -> {
      MySQLSchema updatedSchema = client.resources(MySQLSchema.class)
          .inNamespace(operator.getNamespace()).withName(testSchema.getMetadata().getName()).get();
