...
public class LeaderElectionChangeNamespaceIT {
  ...
  LocallyRunOperatorExtension extension = LocallyRunOperatorExtension.builder()
      ...
      .withReconciler(new LeaderElectionChangeNamespaceReconciler()).build();
  ...
  void noReconcileOnChangeNamespace() {
    ...
    await().pollDelay(Duration.ofSeconds(1)).timeout(Duration.ofSeconds(3)).untilAsserted(() -> {
  ...
  static Lease lease() {
    var lease = new Lease();
    lease
        .setMetadata(new ObjectMetaBuilder().withName(LEASE_NAME).withNamespace("default").build());
    var time = ZonedDateTime.now();
    lease.setSpec(new LeaseSpecBuilder().withAcquireTime(ZonedDateTime.now()).withRenewTime(time)
        .withAcquireTime(time).withHolderIdentity("non-operator-identity").withLeaseTransitions(0)
        .withLeaseDurationSeconds(30).build());
