...
class ResourceRequirementsSanitizerTest {
  ...
  void testSanitizeResourceRequirements_whenContainerSizeMismatch_doNothing() {
    final var template = new PodTemplateSpecBuilder().withNewSpec().addNewContainer()
        .withName("test").endContainer().endSpec().build();
    final var templateWithTwoContainers =
        new PodTemplateSpecBuilder().withNewSpec().addNewContainer().withName("test").endContainer()
            .addNewContainer().withName("test-new").endContainer().endSpec().build();
  ...
  void testSanitizeResourceRequirements_whenResourceIsNull_doNothing() {
    final var template = new PodTemplateSpecBuilder().withNewSpec().addNewContainer()
        .withName("test").endContainer().endSpec().build();
    final var templateWithResource = new PodTemplateSpecBuilder().withNewSpec().addNewContainer()
        .withName("test").withNewResources().endResources().endContainer().endSpec().build();
  ...
  void testSanitizeResourceRequirements_whenResourceSizeMismatch_doNothing() {
    final var actualMap = sanitizeRequestsAndLimits(ContainerType.CONTAINER,
        Map.of("cpu", new Quantity("2")), Map.of(), Map.of("cpu", new Quantity("4")),
  ...
  void testSanitizeResourceRequirements_whenResourceKeyMismatch_doNothing() {
    final var actualMap =
        sanitizeRequestsAndLimits(ContainerType.INIT_CONTAINER, Map.of("cpu", new Quantity("2")),
            Map.of("memory", new Quantity("4Gi")), Map.of(), Map.of());
  ...
  void testSanitizeResourceRequirements_whenResourcesHaveSameAmountAndFormat_doNothing() {
    final var actualMap = sanitizeRequestsAndLimits(ContainerType.CONTAINER,
        Map.of("memory", new Quantity("4Gi")), Map.of("memory", new Quantity("4Gi")),
        Map.of("cpu", new Quantity("2")), Map.of("cpu", new Quantity("2")));
  ...
  void testSanitizeResourceRequirements_whenResourcesHaveNumericalAmountMismatch_doNothing() {
    final var actualMap = sanitizeRequestsAndLimits(ContainerType.INIT_CONTAINER,
        ...
        Map.of("cpu", new Quantity("2")), Map.of("cpu", new Quantity("4000m")));
    assertInitContainerResources(actualMap, "requests").hasSize(2).containsEntry("cpu", "2")
  ...
  void testSanitizeResourceRequirements_whenResourcesHaveAmountAndFormatMismatchWithSameNumericalAmount_thenSanitizeActualMap() {
    final var actualMap = sanitizeRequestsAndLimits(ContainerType.CONTAINER,
        ...
        Map.of("cpu", new Quantity("4")), Map.of("cpu", new Quantity("4000m")));
  ...
  private Map<String, Object> sanitizeRequestsAndLimits(final ContainerType type,
    ...
    sanitizeResourceRequirements(actualMap, actual.getSpec().getTemplate(),
  ...
  private static StatefulSet createStatefulSet(final ContainerType type,
    ...
    if (type == ContainerType.CONTAINER) {
      builder = builder.addNewContainer().withName("test").withNewResources().withRequests(requests)
          .withLimits(limits).endResources().endContainer();
    } else {
  ...
  private static MapAssert<String, Object> assertContainerResources(
    ...
    return assertThat(GenericKubernetesResource.<Map<String, Object>>get(actualMap, "spec",
        "template", "spec", "containers", 0, "resources", resourceName));
  }

  private static MapAssert<String, Object> assertInitContainerResources(
      final Map<String, Object> actualMap, final String resourceName) {
    return assertThat(GenericKubernetesResource.<Map<String, Object>>get(actualMap, "spec",
        "template", "spec", "initContainers", 0, "resources", resourceName));
  }
}
