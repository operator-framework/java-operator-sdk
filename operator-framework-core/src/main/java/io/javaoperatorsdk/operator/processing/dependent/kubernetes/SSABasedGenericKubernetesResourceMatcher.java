 ...
 *
 * <p>
 * The basis of algorithm is to extract the fields managed we convert resources to Map/List composition. The actual
 * resource (from the server) is pruned, all the fields which are not mentioed in managedFields of the target manager is
 * removed. Some irrelevant fields are also removed from desired. And the two resulted Maps are compared for equality.
 * The implementation is a bit nasty since have to deal with some specific cases of managedFields format.
 * </p>
 *
 * @param <R>
...
public class SSABasedGenericKubernetesResourceMatcher<R extends HasMetadata> {
    ...
    private static final List<String> IGNORED_METADATA = Arrays.asList("creationTimestamp", "deletionTimestamp",
    ...
    public boolean matches(R actual, R desired, Context<?> context) {
        ...
        keepOnlyManagedFields(prunedActual, actualMap, managedFieldsEntry.getFieldsV1().getAdditionalProperties(),
                objectMapper);
            ...
            if (claims == actualStatefulSet.getSpec().getVolumeClaimTemplates().size()) {
                for (int i = 0; i < claims; i++) {
                    ...
                    if (desiredStatefulSet.getSpec().getVolumeClaimTemplates().get(i).getStatus() == null) {
                        Optional.ofNullable(GenericKubernetesResource.get(actualMap, "spec", "volumeClaimTemplates", i))
    ...
    @SuppressWarnings("unchecked")
    private static void keepOnlyManagedFields(Map<String, Object> result, Map<String, Object> actualMap,
            ...
            if (key.startsWith(F_PREFIX)) {
                ...
                if (isNestedValue(managedFieldValue)) {
                    ...
                    if (isListKeyEntrySet(managedEntrySet)) {
                        ...
                        fillResultsAndTraverseFurther(result, actualMap, managedFields, objectMapper, key, keyInActual,
                                managedFieldValue);
                    }
                } else {
    ...
    @SuppressWarnings("unchecked")
    private static void fillResultsAndTraverseFurther(Map<String, Object> result, Map<String, Object> actualMap,
            Map<String, Object> managedFields, KubernetesSerialization objectMapper, String key, String keyInActual,
    ...
    /**
     * List entries referenced by key, or when "k:" prefix is used. It works in a way that it selects the target element
     * based on the field(s) in "k:" for example when there is a list of element of owner references, the uid can serve
     * as a key for a list element: "k:{"uid":"1ef74cb4-dbbd-45ef-9caf-aa76186594ea"}". It selects the element and
     * recursively processes it. Note that in these lists the order matters and seems that if there are more keys
     * ("k:"), the ordering of those in the managed fields are not the same as the value order. So this also explicitly
     * orders the result based on the value order in the resource not the key order in managed field.
     */
    @SuppressWarnings("unchecked")
    private static void handleListKeyEntrySet(Map<String, Object> result, Map<String, Object> actualMap,
            KubernetesSerialization objectMapper, String keyInActual, Set<Entry<String, Object>> managedEntrySet) {
        ...
        for (Map.Entry<String, Object> listEntry : managedEntrySet) {
            ...
            var actualListEntry = selectListEntryBasedOnKey(keyWithoutPrefix(listEntry.getKey()), actualValueList,
                    objectMapper);
    ...
    /**
     * Set values, the "v:" prefix. Form in managed fields: "f:some-set":{"v:1":{}},"v:2":{},"v:3":{}} Note that this
     * should be just used in very rare cases, actually was not able to produce a sample. Kubernetes developers who
     * worked on this feature were not able to provide one either when prompted. Basically this method just adds the
     * values from {@code "v:<value>"} to the result.
     */
    @SuppressWarnings("rawtypes")
    ...
    /**
     * Sometimes (not always) the first subfield of a managed field ("f:") is ".:{}", it looks that those are added when
     * there are more subfields of a referenced field. See test samples. Does not seem to provide additional
     * functionality, so can be just skipped for now.
     */
    ...
    @SuppressWarnings("unchecked")
    private static java.util.Map.Entry<Integer, Map<String, Object>> selectListEntryBasedOnKey(String key,
        ...
        if (possibleTargets.size() > 1) {
            throw new IllegalStateException("More targets found in list element for key:" + key + ", in map: "
                    + values.stream().map(Map::keySet).collect(Collectors.toList()));
        }
    ...
    private Optional<ManagedFieldsEntry> checkIfFieldManagerExists(R actual, String fieldManager) {
        var targetManagedFields = actual.getMetadata().getManagedFields().stream()
                ...
                // field manager name.
                .filter(f -> f.getManager().equals(fieldManager) && f.getOperation().equals(APPLY_OPERATION))
                .collect(Collectors.toList());
        if (targetManagedFields.isEmpty()) {
            log.debug("No field manager exists for resource {} with name: {} and operation Apply ", actual.getKind(),
