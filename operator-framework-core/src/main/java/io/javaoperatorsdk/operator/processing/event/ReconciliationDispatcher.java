...
class ReconciliationDispatcher<P extends HasMetadata> {
  ...
  private PostExecutionControl<P> handleDispatch(ExecutionScope<P> executionScope)
    ...
    if (markedForDeletion && shouldNotDispatchToCleanupWhenMarkedForDeletion(originalResource)) {
      log.debug(
          ...
          getName(originalResource), originalResource.getMetadata().getFinalizers());
  ...
  private PostExecutionControl<P> handleReconcile(ExecutionScope<P> executionScope,
  ...
  private PostExecutionControl<P> reconcileExecution(ExecutionScope<P> executionScope,
      P resourceForExecution, P originalResource, Context<P> context) throws Exception {
    log.debug("Reconciling resource {} with version: {} with execution scope: {}",
        getName(resourceForExecution), getVersion(resourceForExecution), executionScope);
  ...
  private PostExecutionControl<P> handleErrorStatusHandler(P resource, P originalResource,
      Context<P> context, Exception e) throws Exception {
  ...
  private PostExecutionControl<P> handleCleanup(P resourceForExecution, P originalResource,
      Context<P> context) {
    if (log.isDebugEnabled()) {
      log.debug("Executing delete for resource: {} with version: {}",
          ResourceID.fromResource(resourceForExecution), getVersion(resourceForExecution));
    }
    ...
    if (useFinalizer) {
      ...
      if (deleteControl.isRemoveFinalizer() && resourceForExecution.hasFinalizer(finalizerName)) {
        P customResource = conflictRetryingPatch(resourceForExecution, originalResource, r -> {
          ...
          if (r == null) {
            log.warn("Could not remove finalizer on null resource: {} with version: {}",
                getUID(resourceForExecution), getVersion(resourceForExecution));
            return false;
          }
    ...
    log.debug(
        ...
        getUID(resourceForExecution), getVersion(resourceForExecution), deleteControl,
        useFinalizer);
  ...
  private P addFinalizerWithSSA(P originalResource) {
    log.debug("Adding finalizer (using SSA) for resource: {} version: {}", getUID(originalResource),
    ...
    try {
      ...
      throw new RuntimeException("Issue with creating custom resource instance with reflection."
          ...
          + originalResource.getClass().getName(), e);
    }
  }

  private P updateCustomResourceWithFinalizer(P resourceForExecution, P originalResource) {
    log.debug("Adding finalizer for resource: {} version: {}", getUID(originalResource),
        getVersion(originalResource));
    ...
    while (true) {
      try {
        ...
        if (retryIndex >= MAX_UPDATE_RETRY) {
          throw new OperatorException("Exceeded maximum (" + MAX_UPDATE_RETRY
              + ") retry attempts to patch resource: " + ResourceID.fromResource(resource));
        }
  ...
  static class CustomResourceFacade<R extends HasMetadata> {
    ...
    public R patchResource(R resource, R originalResource) {
      if (log.isDebugEnabled()) {
        log.debug("Trying to replace resource {}, version: {}", ResourceID.fromResource(resource),
      ...
      if (useSSA) {
        ...
        try {
          ...
          return res.subresource("status")
              .patch(new PatchContext.Builder().withFieldManager(fieldManager).withForce(true)
                  .withPatchType(PatchType.SERVER_SIDE_APPLY).build());
        } finally {
    ...
    public R patchResourceWithSSA(R resource) {
      return resource(resource).patch(new PatchContext.Builder().withFieldManager(fieldManager)
          .withForce(true).withPatchType(PatchType.SERVER_SIDE_APPLY).build());
    }
