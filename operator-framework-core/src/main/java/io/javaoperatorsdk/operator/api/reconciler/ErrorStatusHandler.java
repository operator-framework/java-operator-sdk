...
public interface ErrorStatusHandler<T extends CustomResource<?, ?>> {
  ...
  /**
   * <p>
   * Reconcile can implement this interface in order to update the status sub-resource in the case when
   * the last reconciliation retry attempt is failed on the Reconciler. In that case the updateErrorStatus is
