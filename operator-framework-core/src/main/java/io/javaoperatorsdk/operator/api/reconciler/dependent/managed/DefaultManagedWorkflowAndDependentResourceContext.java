...
public class DefaultManagedWorkflowAndDependentResourceContext<P extends HasMetadata>
  ...
  public <T> T put(Object key, T value) {
    ...
    if (previous != null && !previous.getClass().isAssignableFrom(value.getClass())) {
      logWarning("Previous value (" + previous + ") for key ("
          + key + ") was not of type "
          + value.getClass() + ". This might indicate an issue in your code. If not, use put("
          + key + ", null) first to remove the previous value.");
    }
    return (T) previous;
  }
