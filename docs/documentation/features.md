...
As you can see all the recorded metrics start with the `operator.sdk` prefix. The tags in parenthesis and marked with an
asterisk (`*`) are not used if the implementation is configured to **not** collect metrics per resource.
